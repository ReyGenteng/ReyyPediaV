...
const userSchema = new mongoose.Schema({
  ...
  otpCode: String,
  otpCodeExpired: Date,
  pinTransaksi: { type: String, default: null },
  aktifitas: String,
...
async function verifyPin(req, res, next) {
  try {
    ...
    if (!user || !user.pinTransaksi)
      return res.status(403).json({ success: false, message: "PIN belum diatur" });

    const validPin = await bcrypt.compare(pin, user.pinTransaksi);
    if (!validPin) return res.status(403).json({ success: false, message: "PIN salah" });

    next(); // lanjut ke proses transaksi
  } catch (err) {
...
app.post("/profile/set-pin", requireLogin, async (req, res) => {
  try {
    const { pin, confirmPin } = req.body;

    if (!pin || !confirmPin) {
      return res.status(400).json({ success: false, message: "PIN dan konfirmasi PIN harus diisi" });
    }

    if (pin !== confirmPin) {
      return res.status(400).json({ success: false, message: "PIN tidak cocok" });
    }
    ...
    if (!user.isVerified) {
      return res.status(403).json({ success: false, message: "Verifikasi email dulu sebelum membuat PIN" });
    }

    const hashedPin = await bcrypt.hash(pin, 10);

    user.pinTransaksi = hashedPin;
    await user.save();
...
app.post("/auth/register", async (req, res) => {
  try {
    const { fullname, username, nomor, email, password, referralCode } = req.body;

    // === Validasi unik username/email/nomor ===
    if (await User.findOne({ username })) return res.status(400).json({ error: "Username already exists" });
    if (await User.findOne({ email })) return res.status(400).json({ error: "Email already exists" });
    if (await User.findOne({ nomor })) return res.status(400).json({ error: "Nomor telepon already exists" });

    // === Generate API Key unik ===
    let apiKey = generateApiKey();
    while (await User.findOne({ apiKey })) apiKey = generateApiKey();
    ...
    // === Kalau ada kode referral ===
    if (referralCode) {
      const pengundang = await User.findOne({ referralCode });
      if (pengundang) {
        pengundang.referralHistory.push({
        ...
        pengundang.history.push({
          aktivitas: "User baru mendaftar via referral",
          nominal: 0,
          status: "Pending",
          notes: `Menunggu verifikasi email dari ${newUser.username}`,
          tanggal: new Date(),
        });

        await pengundang.save();
      }
    }
...
app.get("/api/user-otp-info", async (req, res) => {
  const validApiKey = 'DIBOYSTRONGBOYS_RAHASIA_DIBAH'; 
...
app.post("/profile/send-verification-otp", requireLogin, async (req, res) => {
  try {
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail", // bisa diganti: outlook, yahoo, dll
      auth: {
        user: process.env.EMAIL_USER, // isi di .env
        pass: process.env.EMAIL_PASS, // password atau App Password Gmail
      },
    });

    const mailOptions = {
      from: `"ZPAY Verification" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: "Kode Verifikasi Akun ZPAY",
      html: `
    ...
    console.error("Error generate/send OTP:", err);
...
app.post("/profile/verify-email", requireLogin, async (req, res) => {
  const { otp } = req.body;
  if (!otp) return res.status(400).json({ success: false, message: "OTP harus diisi" });
  ...
  try {
    ...
    if (user.referredBy) {
      const pengundang = await User.findOne({ referralCode: user.referredBy });
      if (pengundang) {
        const target = pengundang.referralHistory.find(
          r => r.userId === user._id.toString()
        );
        if (target && target.status === "pending") {
          target.status = "verified";
          pengundang.coin += 500; // 💰 Bonus coin ke pengundang
          pengundang.history.push({
            aktivitas: "Referral terverifikasi",
            nominal: 500,
            status: "Sukses",
            notes: `${user.username} telah verifikasi email.`,
            tanggal: new Date(),
          });
          await pengundang.save();
        }
    ...
    console.error("Error verifikasi email:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
});
